...
// SteeleConfig represents the enhanced steele.json structure
type SteeleConfig struct {
...
// LoadSteeleConfig loads and parses the steele.json file
func LoadSteeleConfig(projectPath string) (*SteeleConfig, error) {
	steeleJsonPath := filepath.Join(projectPath, "steele.json")
	
	data, err := os.ReadFile(steeleJsonPath)
	if err != nil {
		return nil, fmt.Errorf("failed to read steele.json: %w", err)
	}

	var config SteeleConfig
	if err := json.Unmarshal(data, &config); err != nil {
		return nil, fmt.Errorf("failed to parse steele.json: %w", err)
	}

	return &config, nil
}

// GenerateDockerCompose generates a docker-compose.yml from steele.json
func GenerateDockerCompose(projectPath string) error {
	config, err := LoadSteeleConfig(projectPath)
	if err != nil {
		return err
	}
...
// convertService converts a Steele service to Docker Compose service
...
// convertVolume converts a Steele volume to Docker Compose volume
...
// convertNetwork converts a Steele network to Docker Compose network
...
func writeDockerCompose(compose *DockerCompose, filePath string) error {
	...
	// Add header comment
	header := "# Generated by Steele from steele.json\n# Do not edit this file directly - modify steele.json and run 'steele reconfigure'\n\n"
	content := header + string(data)

	return os.WriteFile(filePath, []byte(content), 0644)
}

// AddService adds a new service to steele.json
func AddService(projectPath, serviceName string, service Service) error {
	...
	return saveSteeleConfig(config, projectPath)
}

// RemoveService removes a service from steele.json
...
// saveSteeleConfig saves the steele.json file
func saveSteeleConfig(config *SteeleConfig, projectPath string) error {
	steeleJsonPath := filepath.Join(projectPath, "steele.json")

	data, err := json.MarshalIndent(config, "", "  ")
	if err != nil {
		return fmt.Errorf("failed to marshal steele.json: %w", err)
	}

	return os.WriteFile(steeleJsonPath, data, 0644)
}
